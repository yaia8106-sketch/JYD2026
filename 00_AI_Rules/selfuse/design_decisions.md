...
## Q. DCache Forwarding 寄存器综合/仿真不一致 Bug 修复

**日期**: 2026-04-24
**状态**: 已修复，FPGA 验证通过（current 程序指令条数正确显示 37）

### 问题

current 程序在 FPGA 上运行时，对勾和运行时间正常显示，但**指令条数始终为 00**。
iverilog 仿真（含自定义 counter_stress 测试）全部通过，无法复现。

### 根因

`dcache.sv` 中 3 个 `always_ff @(posedge clk or negedge rst_n)` 块内，
forwarding 数据寄存器（`rf_fwd_way/tag/idx`、`rf_last_fwd_way/addr/data`、`st_fwd_way/addr/data/wea`）
**在 `!rst_n` 分支中没有显式赋值**，只在功能分支中条件赋值。

Vivado 综合报 **Synth 8-7137**: "Register has both Set and reset with same priority. This may cause simulation mismatches."
综合器无法确定这些寄存器的复位行为，生成的硬件与 Verilog 仿真语义不同，
导致 store forwarding 或 refill forwarding 在某些场景下返回错误数据。

### 为什么 iverilog 不受影响

iverilog 严格按 Verilog 语义：未在 `!rst_n` 分支赋值的寄存器**保持原值**。
而 Vivado 综合可能将其推导为带冲突优先级的 set/reset 逻辑，实际硬件行为不确定。

### 修复

为 3 个 always_ff 块中的所有数据寄存器添加显式 `!rst_n` 复位值（共 10 个寄存器）：

```sv
// 修复前
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n)
        rf_fwd_valid <= 1'b0;          // 只复位了 valid
    else if (state == S_DONE) begin
        rf_fwd_way <= rf_way;          // way/tag/idx 无复位 → Synth 8-7137
        ...
