...
WORK_DIR="$VERIFICATION_DIR/common/work/ex_stage_redirect"
...
SIM_BIN="$WORK_DIR/ex_stage_redirect_simv"
COMPILE_LOG="$WORK_DIR/ex_stage_redirect_vcs.log"
SIM_LOG="$WORK_DIR/ex_stage_redirect_sim.log"
...
echo "[INFO] Compiling EX-stage redirect test with VCS..."
# shellcheck disable=SC2086
if ! vcs $VCS_OPTS $VCS_EXTRA_OPTS -top tb_ex_stage_redirect \
    -Mdir="$WORK_DIR/ex_stage_redirect_vcs.csrc" \
...
echo "[INFO] Running EX-stage redirect test..."
if ! "$SIM_BIN" >"$SIM_LOG" 2>&1; then
    cat "$SIM_LOG"
    exit 1
fi
cat "$SIM_LOG"
grep -qF "[PASS] EX-stage redirect source selection" "$SIM_LOG"
