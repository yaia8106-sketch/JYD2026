...
**RV32I + 最小 Zicsr/Trap 五级流水线 + 顺序双发射（In-order Dual-Issue）**
...
- **Slot 0**（主槽）：ALU / Branch / JAL / JALR / Load / Store / CSR / ECALL / MRET — 万能槽
...
- **EX system redirect**：ECALL 目标为 `mtvec` Direct 基址，MRET 目标为当前 `mepc`；系统类 redirect 与 EX fast redirect 共用前端 flush 语义，且不更新分支预测器
...
- **EX system redirect**：ECALL / MRET 使用同一个 `ex_redirect_fire` 条件，确认 EX 指令可前进且不是错误路径后才更新 CSR 状态并重定向前端
