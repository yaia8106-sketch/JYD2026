...
module memory_access_unit (
    ...
    wire dual_issue_cnt_read = (mem_alu_result == DUAL_ISSUE_CNT_ADDR);
    ...
    assign mmio_st_ld_hazard = ex_mem_read_en
                             & mem_valid
                             & (|mem_store_wea)
                             & ~mem_is_cacheable;
    ...
    assign cache_addr = ex_alu_addr;
    assign cache_wea = ex_store_wea;
    assign cache_wdata = ex_store_data;
    assign cache_flush = mem_branch_flush;
    assign cache_pipeline_stall = ~mem_allowin;

    assign mmio_addr = ex_alu_addr;
    ...
    assign mem_load_data = mem_is_cacheable ? cache_rdata : mmio_load_data;
    assign mem_load_ready = mem_ready_go & mem_mem_read_en;
