...
> 2026-05-03 当前进度：已开始 Phase4。已完成 Vivado 工程接线和仿真/综合路径适配：`student_top` 改为两个 IROM 实例，CPU 增加 `irom_addr_plus4`，仿真模型用 `irom_addr`/`irom_addr_plus4` 组成 64-bit fetch window；Vivado 工程补入 Slot1 级间寄存器文件，修正 COE/IP 路径和 IROM pblock。`riscv_tests` 回归 `49/49 PASS`。Vivado check 可跑完 synthesis/place/route，但 200MHz timing 尚未闭合，当前最好 routed WNS = `-0.189ns`，剩余关键路径主要是 IROM BRAM 输出经双发射/取指控制回到 IROM BRAM 地址。
...
### 改动清单
- [x] Vivado 综合，分析时序报告
- [ ] 根据关键路径优化（已尝试 PC+4/+8/+12 预计算、`irom_addr_plus4`、保守 RAW；仍需继续收敛）
- [ ] FPGA 烧录

### 验证标准
- [ ] 时序收敛 ≥ 200MHz（WNS ≥ 0；当前最好 WNS = `-0.189ns`）
