...
module forwarding (
    ...
    // Keep one explicit cone per architectural operand. Vivado maps this
    // representation to four independent mux trees and can retain the
    // dedicated MUXF7/MUXF8 resources used by the timing-tuned implementation.
    // The repeated structure is intentional: an indexed operand array caused
    // cross-operand factoring and increased local ID/EX routing pressure.
    ...
    // EX match even if unused instruction bits happen to equal the producer rd.
    ...
    // ================================================================
    //  Dependency policy integration
    // ================================================================
    // Repair is a one-cycle promise: the consumer advances now and substitutes
    // the registered load result in EX on the following cycle. Conditional
    // branches repair only compare operands; JIRL waits because rs1 also forms
    // its redirect target.
    ...
    // These wires intentionally remain in the forwarding scope because the
    // performance and Konata monitors observe the established hierarchy.
    ...
    // Kept as named compatibility probes. EX-produced control operands now
    // use ordinary forwarding and resolve in EX; Slot-1 WB is also forwarded.
