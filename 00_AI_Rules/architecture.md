# 双发射 CPU 架构文档（从 RTL 反向生成，2026-05-20 更新）
...
**RV32IM + 最小 Zicsr/Trap 五级流水线 + 顺序双发射（In-order Dual-Issue）**
...
- **Slot 0**（主槽）：ALU / Branch / JAL / JALR / Load / Store / RV32M / CSR / ECALL / MRET — 万能槽
- **Slot 1**（副槽）：ALU（R-type / I-ALU / LUI / AUIPC）+ 条件分支（B-type）
- 两 Slot 同步推进，共享 `allowin`，不可独立 stall
- CSR / ECALL / MRET 归类为系统类顺序化指令，只允许 Slot 0 执行；Slot 0 为系统类时 Slot 1 不发射，系统类出现在 Slot 1 位置时进入后续周期作为 Slot 0 执行。
...
- **MEM replay**：`mem_branch_even_addr_r` / `mem_branch_odd_addr_r`（寄存器）；覆盖 slot0 branch/JAL/JALR 误预测、slot1 taken branch 的延迟 redirect，以及 target mismatch
- **EX system redirect**：ECALL 目标为 `mtvec` Direct 基址，MRET 目标为当前 `mepc`；系统类 redirect 仍走同拍前端 flush，且不更新分支预测器
...
- 当前前端采用 aggressive sequential fetch：`predict_dual` 复位为 `1'b0`，首拍取 `+4` 以确保第一条真实指令从 `0x8000_0000` 开始；首次有效取指后保持 `1'b1`，正常顺序路径优先取下一 64-bit 窗口（`+8`）。
...
- 系统类指令不进入同包 RAW 例外路径：Slot0 为 CSR / ECALL / MRET 时直接阻止 Slot1 发射。
...
| RV32M | ❌ 不可以 | M 扩展操作只在 Slot0 执行；位于 Slot1 位置时进入后续周期作为 Slot0 |
| CSR / ECALL / MRET | ❌ 不可以 | 系统类顺序化指令只在 Slot0 执行，且 Slot0 为系统类时不发射 Slot1 |
...
### 4.4 分支目标预计算与 registered replay
...
`id_ex_reg` 额外传递 `ex_branch_target` 和 `ex_fallthrough_pc`。EX 级 `branch_unit` 只做条件结果和预测对比，并输出 `branch_flush` 与正确 `branch_target`。为收敛 200MHz，slot0 branch/JAL/JALR redirect 不再同拍驱动 `irom_addr`；它先写入 `ex_mem_reg` 的 registered flush/target，下一拍通过 `mem_branch_replay` 修正前端。

```systemverilog
ex_redirect_fire = ~mem_branch_flush & ex_ready_go_w & mem_allowin;
ex_branch_registered_flush = branch_flush & ex_redirect_fire & ~ex_system_inst;
mem_branch_replay = mem_branch_flush & ~fast_branch_redirect_r;
frontend_branch_flush = mem_branch_replay | ex_system_redirect;
```

- `ex_branch_registered_flush` 只在 EX 指令可前进时产生，避免 JAL/JALR 被前端 flush 提前杀掉而丢失 link 写回。
- `mem_branch_flush` 由 `ex_mem_reg` 打拍，下一拍驱动 `pc_reg` / `irom_addr` / IF-ID flush / ID-EX flush。
- `fast_branch_redirect_r` 只抑制已被同拍消费的系统 redirect 的重复 replay；普通分支不走同拍 IROM 快路径。
- 这个 tradeoff 给 branch/JAL/JALR 误预测多 1 个周期，但切断 `mem_allowin/PC -> IROM BRAM` 最差路径。

slot1 条件分支同样只走 registered replay。它在 EX 级用 `branch_cond_taken(ex_s1_rs1_data, ex_s1_rs2_data, ex_s1_branch_cond)` 解析，target 为 `alu_s1_result`，并在 taken 且 target 不等于 `ex_s1_pc_plus_4` 时生成 `ex_s1_branch_redirect`。该 redirect 与 slot0 registered branch/system redirect 一起写入 `ex_mem_reg` 的 registered flush/target；下一拍通过 `mem_branch_replay` 修正前端。若 slot0 同拍也 redirect，slot0 target 优先。
...
- `id_ex_reg` 额外传递 `ex_branch_target` / `ex_fallthrough_pc`，供 EX registered replay 使用，并传递 `ex_is_muldiv/ex_muldiv_op`
...
`muldiv_unit.sv` 实现完整 RV32M：`MUL/MULH/MULHSU/MULHU/DIV/DIVU/REM/REMU`。

- M 指令由 `decoder` 识别 `funct7=7'b0000001`，只允许 Slot0 执行；Slot1 位置遇到 M 指令时通过 `dual_issue_decider` 留到后续周期作为 Slot0。
- 乘法使用 33-bit 操作数扩展并带 `(* use_dsp = "yes" *)`，综合映射为 4 个 DSP48E1。输入先打一拍到 `mul_a_r/mul_b_r`，再打一拍得到 `mul_product_r`，避免 WB/forwarding 修复结果直接进入 DSP 组合路径。
- 除法/取余采用 radix-2 迭代除法，32 次迭代完成，面积低且易收敛；除零和有符号溢出按 RISC-V 规范返回。
- EX 级遇到 M 指令时通过 `ex_muldiv_ready` stall，直到 `muldiv_done`；M 结果进入现有 EX/MEM/WB 和 forwarding 路径。
- `perf_monitor` 统计 `MUL/DIV wait`，用于把 M 扩展带来的等待周期和 DCache/MMIO stall 区分开。

---

## 8. CSR 与 Trap

`cpu_top` 内部实现 M 模式最小 CSR 状态集合：
...
- 支持 Zicsr 六类读改写行为：CSRRW / CSRRS / CSRRC / CSRRWI / CSRRSI / CSRRCI。
- CSR 读旧值作为 Slot0 普通写回数据进入现有 WB/forwarding 路径；CSR 写在 EX 指令确认有效且可前进时生效。
- CSRRW/CSRRWI 即使源值为 0 也写 CSR；CSRRS/CSRRC 在 `rs1=x0` 时只读不改；CSRRSI/CSRRCI 在 `uimm=0` 时只读不改。
- 未支持 CSR 采用宽松策略：读零、写忽略，不触发非法指令异常。
- ECALL 在 EX 级生成系统 redirect：`mepc <= ex_pc`，`mcause <= 11`，`MPIE <= MIE`，`MIE <= 0`，目标为 `mtvec` Direct 基址。
- MRET 在 EX 级生成系统 redirect：目标为当前 `mepc`，`MIE <= MPIE`，`MPIE <= 1`。
- CSR / ECALL / MRET 不作为普通 branch/JAL/JALR 更新 BTB、BHT、GShare、Selector 或 RAS。
...
- `jalr_ex_wait_hazard` / `branch_ex_wait_hazard`：避免 EX/S1_EX 结果直接进入 ID 级 JALR target 或分支比较，保护前端时序。

Load-use 只对指令实际使用的源操作数生效：
- S0 rs1：`id_rs1_used = (dec_alu_src1_sel == 2'b00) | dec_is_branch | dec_csr_uses_rs1`
...
- **EX 级**：`branch_unit` 组合产生 `branch_flush`（方向错 / 目标错）和正确 `branch_target`
- **Registered branch replay**：`ex_branch_registered_flush = branch_flush & ex_redirect_fire & ~ex_system_inst`，写入 EX/MEM；下一拍 `mem_branch_replay` 才驱动前端 flush/redirect
- **EX system redirect**：ECALL / MRET 仍可同拍驱动前端 flush；确认 EX 指令可前进且不是错误路径后才更新 CSR 状态并重定向前端
- **Slot1 branch delayed redirect**：`ex_s1_branch_redirect` 只在 slot1 条件分支 taken 时产生，不驱动当拍前端；它和 slot0 registered redirect 一起写入 EX/MEM registered flush/target
- **MEM 级**：`mem_branch_flush` = `ex_registered_branch_flush` 打拍；普通分支/JAL/JALR/slot1 branch 在这一拍通过 replay 修正前端
- **前端 replay 抑制**：`mem_branch_replay = mem_branch_flush & ~fast_branch_redirect_r`，当前只抑制已同拍处理过的系统 redirect
- **Flush 范围**：
...
- `id_ready_go = ~(load_use_hazard | repair_use_hazard | jalr_ex_wait_hazard | branch_ex_wait_hazard)`
- `ex_ready_go = ~mmio_st_ld_hazard & ex_muldiv_ready`（保守：EX load + MEM MMIO store → 1 cycle stall；M 指令等待 `muldiv_done`）
...
| `decoder` ×2 | 指令译码（S0 / S1） | 完整 RV32IM + Zicsr/Trap 译码 |
...
| `muldiv_unit` | RV32M 乘除单元 | DSP 乘法 + 32-cycle 迭代除法，EX 多周期 stall |
| `branch_condition` | Branch 条件比较 | 复用 RV32 B-type 条件判断，供 ID 预计算和 S1 branch 使用 |
| `branch_unit` | 分支判断 + 误预测检测 | 只处理 S0，使用 ID 预计算 target/fallthrough，输出 registered replay 所需 target |
| `redirect_ctrl` | 前端 redirect/flush 控制 | 生成系统 fast redirect、MEM branch replay、`frontend_branch_flush/target` |
