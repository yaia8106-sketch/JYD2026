...
def write_mem(path: Path, words: List[int], depth: int, fill: int) -> None:
    if len(words) > depth:
        raise ValueError(f"{path.name} needs {len(words)} words, depth is {depth}")
    padded = words + [fill] * (depth - len(words))
...
def split_or_read_irom(coe_dir: Path) -> Tuple[List[int], List[int]]:
    ...
    raise FileNotFoundError(f"cannot find irom_slot0/1.coe or irom.coe under {coe_dir}")
...
def main() -> int:
    ...
    parser.add_argument("--rom-depth-words", type=int, default=2048)
    parser.add_argument("--dram-depth-words", type=int, default=65536)
    args = parser.parse_args()

    workspace = Path(args.workspace).resolve()
    project_dir = workspace / "PhysicalTwin_Nexys4DDR"
    out_dir = project_dir / "generated"
    out_dir.mkdir(parents=True, exist_ok=True)

    coe_arg = Path(args.coe)
    coe_dir = coe_arg if coe_arg.is_absolute() else workspace / "02_Design" / "coe" / args.coe
