#!/bin/bash
# ============================================================
# run_abtb.sh - Standalone VCS functional test for frontend_abtb
# ============================================================
...
SIM_BIN="$WORK_DIR/frontend_abtb_simv"
COMPILE_LOG="$WORK_DIR/frontend_abtb_vcs.log"
SIM_LOG="$WORK_DIR/frontend_abtb_sim.log"
...
echo "[INFO] Compiling frontend ABTB directed test with VCS..."
# shellcheck disable=SC2086
if ! vcs $VCS_OPTS $VCS_EXTRA_OPTS -top tb_frontend_abtb \
    -Mdir="$WORK_DIR/frontend_abtb_vcs.csrc" \
...
head -20 "$COMPILE_LOG"
echo "[INFO] Running frontend ABTB directed test..."
if ! "$SIM_BIN" >"$SIM_LOG" 2>&1; then
    cat "$SIM_LOG"
    exit 1
fi
cat "$SIM_LOG"
if ! grep -qF "[PASS] frontend_abtb directed test" "$SIM_LOG"; then
    echo "ERROR: frontend ABTB simulation did not report PASS"
    exit 1
fi
