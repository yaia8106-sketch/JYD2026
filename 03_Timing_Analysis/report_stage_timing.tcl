...
foreach from_grp $all_groups {
    ...
    foreach to_grp $all_groups {
        ...
        foreach p $paths {
            ...
            log_msg [format "    %-5d  %10s  %10s  %6s    %s" \
                $idx [format_ns $p_slack] [format_ns $p_data] [format_levels $p_lvl] $end_short]
        }
    }
}
...
log_msg ""
log_msg [string repeat "=" 80]
log_msg "\n\[Step 5\] 全局最差路径 Top-${GLOBAL_MAX_PATHS}（不受分组限制）\n"
log_msg "  用于发现未被寄存器组分类覆盖的关键路径\n"

set global_paths [get_timing_paths -quiet \
    -max_paths $GLOBAL_MAX_PATHS \
    -nworst $GLOBAL_MAX_PATHS \
    -sort_by slack]
if {[llength $global_paths] > 0} {
    ...
    foreach gp $global_paths {
        ...
        log_msg [format "  %-4d  %10s  %10s  %6s  %-40s  %s %s" \
            $gidx [format_ns $g_slack] [format_ns $g_data] [format_levels $g_lvl] $g_start $g_end $g_status]
    }
} else {
    log_msg "  （未找到时序路径）"
}
