...
`functional/run_all.sh` 默认配置为运行 11 个独立 RTL/前端定向测试和 89 个处理器程序。
这里描述的是覆盖规模，不等同于当前工作区已经通过这些测试。

- 5 个核心/存储定向测试：forwarding、MulDiv 随机、Bitmanip
  解码/执行随机、DRAM IP 延迟约束和 store-buffer 状态/查询/refill。
...
`functional/frontend/run_abtb.sh`、`run_direction.sh`、`run_integration.sh`、
