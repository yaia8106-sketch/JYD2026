...
SIM_BIN="$WORK_DIR/muldiv_simv"
COMPILE_LOG="$WORK_DIR/muldiv_vcs.log"
SIM_LOG="$WORK_DIR/muldiv_sim.log"
...
echo "[INFO] Compiling muldiv randomized test with VCS..."
# shellcheck disable=SC2086
if ! vcs $VCS_OPTS $VCS_EXTRA_OPTS -top tb_muldiv_unit \
    -Mdir="$WORK_DIR/muldiv_vcs.csrc" \
    -o "$SIM_BIN" \
    "$RTL_DIR/common/cpu_defs.sv" \
    "$RTL_DIR/core/execute/muldiv_unit.sv" \
    "$SCRIPT_DIR/tb_muldiv_unit.sv" \
...
echo "[INFO] Running muldiv randomized test..."
if ! "$SIM_BIN" >"$SIM_LOG" 2>&1; then
    cat "$SIM_LOG"
    exit 1
fi
cat "$SIM_LOG"
grep -qF "[PASS] muldiv unit randomized test" "$SIM_LOG"
