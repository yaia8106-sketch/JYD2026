...
- `run_vivado_flow.tcl` 的 COE 参数默认仍是 `current`，但会优先解析到 `02_Design/verification/riscv/coe/irom64/current`
- **VCS COE 性能/板级仿真**默认使用 `dual_issue/` 下的 `irom_slot0.coe`、`irom_slot1.coe` 和 `dram.coe`，入口是 `02_Design/verification/riscv/run_coe_perf.sh`、`run_student_top_coe.sh`
