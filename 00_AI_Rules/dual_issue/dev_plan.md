...
> 2026-05-04 当前进度：Phase4 Vivado 路径已切换为两个 32-bit IROM bank（`IROMEven32`/`IROMOdd32`），由 Tcl 自动把 `irom.coe` 拆成 even/odd 两份；CPU 侧移除 `irom_addr_plus4`，`student_top` 用一拍对齐的 bank select 重新组成 64-bit fetch window。`riscv_tests` 回归 `60/60 PASS`。Vivado `check current 18` 可跑完 synthesis/place/route，综合和实现 DRC 已无 IROM 组合环；200MHz timing 尚未闭合，当前 routed WNS = `-0.886ns`，主要剩余路径仍集中在 IROM BRAM 输出/取指控制回到 IROM BRAM 地址。
...
### 改动清单
- [x] Vivado 综合，分析时序报告
- [ ] 根据关键路径优化（已尝试 PC+4/+8/+12 预计算、banked IROM、保守 RAW；仍需继续收敛）
- [ ] FPGA 烧录

### 验证标准
- [ ] 时序收敛 ≥ 200MHz（WNS ≥ 0；当前 banked IROM WNS = `-0.886ns`）
